...
5. Release captures one finite per-consumer/per-lane watermark/count cut. Drainers run under opaque, generation-bound permits in the exported order `failure-attention-decision`, `mesh-reply`, `mesh-unsolicited`, `subagent-success`, `background-notify`, `loop-tick`, then `cron-tick`. A consumer may own multiple lanes, and each acknowledgement must match the exact consumer, lane, handled watermark, and count within five seconds. In V1, a successful non-empty acknowledgement represents exactly one `sendMessage(..., { triggerTurn: true })` submission.
6. After each submitted acknowledgement, the coordinator holds the frozen cut until the next genuine same-generation `agent_settled` boundary before invoking another drainer. The operation returns to idle only after every captured lane is empty or its one submitted turn has settled, or an explicit block is recorded.
...
A normal tool or legacy registry request always waits for the next genuine `agent_settled`. The additive `current-or-next-settled-boundary` policy is reserved for the fixed paired Remote adapter attestation (`reason:"remote"`, `actor:"operator"`, `channel:"remote"`, `source:"remote-pi-action"`). The lifecycle owner—not Remote—keeps a generation-bound `session_start`/`agent_settled` proof, invalidates it at public activity and managed-wake reservation seams, and starts synchronously only when that proof is current and the public context is idle with no pending messages. Missing proof fails closed; `isIdle()` alone never authorizes compaction. Joining an existing operation never activates this path, so tool-result settlement semantics remain unchanged.
